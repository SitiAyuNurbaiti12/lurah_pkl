...
<div class="container" style="font-family: time;">
  ...
  <br>
	...
	<h2 class="text-center"> Surat Keterangan Tidak Mampu</h2><br>
	<table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
			<thead class="">
				...
				if (isset($_POST['cetak'])) {
					...
					$result = mysqli_query($koneksi, "SELECT * FROM pemohon INNER JOIN pj_tmampu ON pemohon.id_pm=pj_tmampu.id_pm INNER JOIN rg_tmampu ON pj_tmampu.id_pj_ktm=rg_tmampu.id_pj_ktm INNER JOIN pegawai ON pegawai.id_pg=rg_tmampu.id_pg WHERE rg_tmampu.tgl_rg_ktm LIKE '%$bulan%' AND rg_tmampu.tgl_rg_ktm LIKE '%$tahun%'");
				}else{
					$result = mysqli_query($koneksi, "SELECT * FROM pemohon INNER JOIN pj_tmampu ON pemohon.id_pm=pj_tmampu.id_pm INNER JOIN rg_tmampu ON pj_tmampu.id_pj_ktm=rg_tmampu.id_pj_ktm INNER JOIN pegawai ON pegawai.id_pg=rg_tmampu.id_pg");
				}
				if(mysqli_num_rows($result)==0) {
					echo "<script>alert('Data Tidak Ada!.'); window.location = '../admin/rg_tmampu.php';</script>";; 	
				}
				$i=1;
				while($row = mysqli_fetch_array($result)) { ?>
					<tr class="text-center">
						...
						<td><?= $row['jk_pmh'] ?></td>
						<td><?= $row['pekerjaan_ktm'] ?></td>
						<td><?= $row['no_hp'] ?></td>
						<td><?= $row['alamat_ktm'] ?></td>
						<td><?= $row['tujuan_ktm'] ?></td>
						<td><?= tgl_indo($row['tgl_rg_ktm']) ?></td>
					</tr>
				<?php } ?>
			</tbody>
		</thead>
