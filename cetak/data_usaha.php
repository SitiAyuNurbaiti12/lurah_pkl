...
<div class="container" style="font-family: time;">
  ...
  <br>
	...
	<h2 class="text-center"> Surat Keterangan Usaha</h2><br>
	<table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
			<thead class="">
				...
				if (isset($_POST['cetak'])) {
					...
					$result = mysqli_query($koneksi, "SELECT * FROM pemohon INNER JOIN pj_usaha ON pemohon.id_pm=pj_usaha.id_pm INNER JOIN rg_usaha ON pj_usaha.id_pj_ush=rg_usaha.id_pj_ush INNER JOIN pegawai ON pegawai.id_pg=rg_usaha.id_pg WHERE rg_usaha.tgl_rg_ush LIKE '%$bulan%' AND rg_usaha.tgl_rg_ush LIKE '%$tahun%'");
				}else{
					$result = mysqli_query($koneksi, "SELECT * FROM pemohon INNER JOIN pj_usaha ON pemohon.id_pm=pj_usaha.id_pm INNER JOIN rg_usaha ON pj_usaha.id_pj_ush=rg_usaha.id_pj_ush INNER JOIN pegawai ON pegawai.id_pg=rg_usaha.id_pg");
				}
				if(mysqli_num_rows($result)==0) {
					echo "<script>alert('Data Tidak Ada!.'); window.location = '../admin/rg_usaha.php';</script>";; 	
				}
				$i=1;
				while($row = mysqli_fetch_array($result)) { ?>
					<tr class="text-center">
						...
						<td><?= $row['alamat'] ?></td> 
						<td><?= $row['jenis_ush'] ?></td>  
						<td><?= $row['alamat_ush'] ?></td>
						<td><?= tgl_indo($row['tgl_rg_ush']) ?></td>  
					</tr>
				<?php } ?>
			</tbody>
		</thead>
