        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Verifikasi Surat Keterangan Tidak Mampu</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                    ...
                    <td id="ikonbtn2">
                      <form action="" method="post">
                        <div class="text-center">
                          <input type="text" class="form-control mb-2" name="status_ktm" placeholder="Isi Keterangan ACC/Tolak" required="">
                          ...
                          <input type="hidden" class="form-control" name="id_pj_ktm" id="inputName" value="<?=$data['id_pj_ktm'];?>"> 
                          </button>
                        </div>
                      </form>
                    </td>
...
<?php 
  if (isset($_POST['tolak'])) {
    ...
    $status_ktm = $_POST['status_ktm'];
    $status_surat = $_POST['status_surat'];
    $update = mysqli_query($koneksi, "UPDATE pj_tmampu SET status_ktm = '$status_ktm', status_surat = '2' WHERE id_pj_ktm = '$id_pj_ktm'");
if($update){
          ?> <script>alert('Berhasil Disimpan!'); window.location = 'vr_tmampu.php';</script><?php
        }else{
          ?> <script>alert('Gagal, cek kembali!.'); window.location = 'vr_tmampu.php';</script><?php
        }  
  }
    ...
    $status_ktm = $_POST['status_ktm'];
    $status_surat = $_POST['status_surat'];
    $update = mysqli_query($koneksi, "UPDATE pj_tmampu SET status_ktm = '$status_ktm', status_surat = '1' WHERE id_pj_ktm = '$id_pj_ktm'");
if($update){
          ?> <script>alert('Berhasil Disimpan!'); window.location = 'vr_tmampu.php';</script><?php
        }else{
          ?> <script>alert('Gagal, cek kembali!.'); window.location = 'vr_tmampu.php';</script><?php
