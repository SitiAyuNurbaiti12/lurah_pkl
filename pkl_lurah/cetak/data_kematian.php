  ...
  <br>
	...
	<table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
			<thead class="">
				...
				$i=1;
				$sae = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM pemohon INNER JOIN pj_kematian ON pemohon.id_pm=pj_kematian.id_pm INNER JOIN rg_kematian ON pj_kematian.id_pjkt=rg_kematian.id_pjkt INNER JOIN pegawai ON pegawai.id_pg=rg_kematian.id_pg"));
				$total = 0;$total2 = 0;
				...
				<tr class="text-center">
			      <td colspan="7">
					...
					$explode=explode("-",$str);  
					?>
			        Total Kematian <!-- <?php echo tgl_indo($explode[1]);?> --> :
			      </td>
			      <td colspan="2">
			        <?=$total?> Orang
			      </td>
